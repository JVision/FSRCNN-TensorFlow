    ...
    for i in range(1, m+1):
        with tf.variable_scope("FMM{}".format(i)) as scope:
            for bi in range(1, b+1):
                ...
                sa = tf.contrib.layers.conv2d(res, d*2, 1, 1, 'SAME', 'NHWC', activation_fn=tf.nn.leaky_relu, scope='sa_widen{}'.format(bi))
                sa = tf.contrib.layers.conv2d(sa, 1, 1, 1, 'SAME', 'NHWC', activation_fn=tf.sigmoid, scope='sa{}'.format(bi))

                ca = tf.reduce_mean(res, axis=[1, 2], keepdims=True)
                ca = tf.contrib.layers.conv2d(ca, max(d//16, 4), 1, 1, 'SAME', 'NHWC', activation_fn=tf.nn.leaky_relu, scope='ca_shrink{}'.format(bi))
                ca = tf.contrib.layers.conv2d(ca, d, 1, 1, 'SAME', 'NHWC', activation_fn=tf.sigmoid, scope='ca{}'.format(bi))

                res = tf.concat([tf.multiply(res, sa), tf.multiply(res, ca)], -1)
                res = tf.contrib.layers.conv2d(res, d, 1, 1, 'SAME', 'NHWC', activation_fn=None, scope='csar{}'.format(bi))
